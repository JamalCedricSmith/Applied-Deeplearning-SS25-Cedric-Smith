...
def split_dataset(input_file: str, train_ratio: float = 0.9, balance_train: bool = True):
    ...
    if balance_train:
        ...
        train_cases = [
            item[0]["video_filename"].split("_")[2]
            for item in train_data
            if isinstance(item[0].get("video_filename"), str)
            and item[0]["video_filename"].count("_") >= 2
        ]

        # Won't count idle phases
        ...
        case_to_samples = {}
        for item in train_data:
            fn = item[0].get("video_filename") if item and isinstance(item[0], dict) else None
            if isinstance(fn, str) and fn.count("_") >= 2:
                case = fn.split("_")[2]
                case_to_samples.setdefault(case, []).append(item)
            # else: skip invalid/missing filenames
...
if __name__ == "__main__":
    input_file = "datasets/cataract1k/qa_pairs_graph.json"
