...
if __name__ == "__main__":
    ...
    ## for case, annotation in zip(cases, annotations):
    for case, annotation in zip(cases, annotations):
        ann_df = pd.read_csv(annotation).sort_values("sec").reset_index(drop=True)
        video_file = os.path.join(video_folder, "videos_224", f"{case}.mp4")

        # idle from t=0 to first phase start
        first_start = float(ann_df.loc[0, "sec"])
        if first_start > 1e-3:
            cut_video(video_file, output_folder, "idle", 0.0, first_start)

        # idle between consecutive phases
        for i in range(1, len(ann_df)):
            gap_start = float(ann_df.loc[i - 1, "endSec"])
            gap_end   = float(ann_df.loc[i, "sec"])
            if gap_end - gap_start > 1e-3:
                cut_video(video_file, output_folder, "idle", gap_start, gap_end)
